...
{
	ConfigSet S1Config;
	string ConfigPath="I:\\MyCode\\Sentinel-1_Demo\\Release\\config.txt";
	ReadInput(S1Config, ConfigPath.c_str());
	 ...
	 RefDem& TopoDem, TransFormCoef TFCoef, int Mburst0, int MburstN, int Sburst0, int SburstN ,int SwathId)
 {
	 ...
	 for (int burstId = Mburst0; burstId <= MburstN; burstId++)
	 {
		 ...
		 TopoDem.getData(lat_min, lat_max, lon_min, lon_max, extralat, extralon,
		  ...
		  {
			  cout << "CpmAz:" << (TFCoef.getAzCoeff(burstId))[iter] << endl;
			 
		  }
		  for (int iter = 0; iter < 6; iter++)
		  {
			  cout << "CpmRg:" << (TFCoef.getRgCoeff(burstId))[iter] << endl;
		  }
		  system("pause");*/
